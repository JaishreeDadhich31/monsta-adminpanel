...
export default function AddSubCategory() {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        setSubCategoryId(params.id);                  //params update ke liye

        if (params.id) {
            axios.post(`${import.meta.env.VITE_API_BASE_URL}/sub-category/details/${params.id}`)
                .then((result) => {
                    if (result.data._status) {
                        setSubCategorylDetails(result.data._data)
    ...
    let formhandler = (event) => {
        ...
        if (newErrors.length === 0) {
            ...
            if (subCategoryId) {
                axios.put(`${import.meta.env.VITE_API_BASE_URL}/sub-category/update/${subCategoryId}`,
                    event.target)
                    .then((result) => {
                        console.log(result.data)
                        if (result.data._status == true) {
                            event.target.reset()
                            pageNavigate('/subcategory/view-category')
                            iziToast.success({
                                title: 'Success',
                                message: result.data._message,
                                position: 'topRight'
                ...
                axios.post(`${import.meta.env.VITE_API_BASE_URL}/sub-category/create`, 
                    event.target
                )
                    .then((result) => {
                        if (result.data._status == true) {
      ...
      <div className="min-h-screen bg-gray-100 ml-64 p-6 rounded-lg">
        ...
        <div className="bg-white border-b px-6 py-4">
          <p className="text-2xl font-semibold text-gray-800">
            Home | Sub | <span className='text-violet-500'>
              {
